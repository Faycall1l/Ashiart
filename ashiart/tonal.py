...
def apply_clahe(gray: Image.Image, tiles: int = 8, clip: float = 2.0) -> Image.Image:
    ...
    padded = np.pad(
        source, ((0, rows * tile_h - height), (0, cols * tile_w - width)), mode="edge"
    )
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            hist = (
                np.minimum(hist, limit)
                + max(np.maximum(hist - limit, 0).sum(), 0) / 256
            )
            cdf = hist.cumsum()
            positive = cdf[cdf > 0]
            denom = tile.size - positive[0] if positive.size else 0
            luts[i, j] = (
                (cdf - positive[0]) / denom * 255 if denom > 0 else np.arange(256)
            )
...
def apply_dog(
    gray: Image.Image, small: float = 0.8, large: float = 2.0, amplify: float = 2.0
) -> Image.Image:
    ...
    return Image.fromarray(
        np.clip(np.round(frame + amplify * detail), 0, 255).astype(np.uint8), mode="L"
    )
